...
public class PlantInfo {
	...
	public List<Plant> getPlantsByScientificName(String scientificName) {
		return mPlants.stream().filter(
				(plantLHS)-> {
					String[] namesLHS = plantLHS.getScientificName().split("\\W+");
					String[] namesRHS = scientificName.split("\\W+");
					for (int i = 0; i < namesLHS.length; i++) {
						if (!namesLHS[i].startsWith(namesRHS[i]))
							return false;
					}
					return true;
